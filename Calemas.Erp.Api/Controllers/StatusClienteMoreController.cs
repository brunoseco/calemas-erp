...
{
	[Authorize]
    [Route("api/statuscliente/more")]
    public class StatusClienteMoreController : Controller
    {

        private readonly IStatusClienteRepository _rep;
        private readonly IStatusClienteApplicationService _app;
		private readonly ILogger _logger;

        public StatusClienteMoreController(IStatusClienteRepository rep, IStatusClienteApplicationService app, ILoggerFactory logger)
        {
            this._rep = rep;
            this._app = app;
			this._logger = logger.CreateLogger<StatusClienteMoreController>();
        }
        ...
        public async Task<IActionResult> Get([FromQuery]StatusClienteFilter filters)
        {
            ...
            try
            {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "StatusCliente");
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<StatusClienteDtoSpecialized> dtos)
        {
            ...
            {
                return result.ReturnCustomException(ex,"Calemas.Erp - StatusCliente", dtos);
            }

        }

		[HttpPut]
        public async Task<IActionResult> Put([FromBody]IEnumerable<StatusClienteDtoSpecialized> dtos)
        {
            var result = new HttpResult<StatusClienteDto>(this._logger);
            try
            {
