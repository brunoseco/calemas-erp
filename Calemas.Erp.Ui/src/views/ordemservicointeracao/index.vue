                        ...
                        <table class="table has-tickbox table-striped table-sm">
                            ...
                            <tbody>
                                <tr v-for="item in result.itens" class="animated fadeIn">
                                    <td>{{ item.ordemServicoInteracaoId }}</td>
                                    <td>{{ item.dataConclusao }}</td>
                                    <td>{{ item.descricao }}</td>
                                    <td>{{ item.observacao }}</td>
                                    <td><span class="badge badge-pill" v-bind:class="{ 'badge-success': item.foiProprioCliente, 'badge-danger': !item.foiProprioCliente }">{{item.foiProprioCliente ? 'Sim' : 'Não'}}</span></td>
                                    <td>{{ item.nomeClienteResponsavel }}</td>
...
<script>
    ...
    export default {
        name: "ordemservicointeracao",
        components: { formPartial, filterPartial },
        mixins: [crudBase],
        data() {
            return {
                resource: "ordemservicointeracao",
            }
        }
    }
</script>