<template>
    <div class="wrapper">
        ...
        <div class="row" style="margin-bottom: 1rem;margin-top: -0.5rem;">
            <div class="col-xs-12 col-sm-7 col-md-7 col-lg-8">
                <h6 class="page-title txt-color-blueDark">
                    ...
                    Cadastros
                    <span>Manutenção de Situação de Pagamento</span>
                </h6>
            </div>
                        ...
                        <table class="table has-tickbox table-striped table-sm">
                            ...
                            <tbody>
                                <tr v-for="item in result.itens" class="animated fadeIn">
                                    <td>{{ item.statusPagamentoId }}</td>
                                    <td>{{ item.nome }}</td>

                                    <td class="text-center">
                                        <button type="button" class="btn btn-sm btn-primary" v-tooltip.left="'Editar'" @click="openEdit(item.statusPagamentoId, item)">
                                            <i class="fa fa-pencil"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-danger" v-tooltip.left="'Remover'" @click="openDelete(item.statusPagamentoId, item)">
        ...
        <modal title="Cadastro de Situação de Pagamento" v-model="modalCreateIsOpen" effect="fade/zoom" type="modal-success" :large="true">
            <div slot="modal-header" class="modal-header">
                <h4 class="modal-title">Cadastro de Situação de Pagamento</h4>
        ...
        <modal title="Edição de Situação de Pagamento" v-model="modalEditIsOpen" effect="fade/zoom" type="modal-primary" :large="true">
            <div slot="modal-header" class="modal-header">
                <h4 class="modal-title">Edição de Situação de Pagamento</h4>
        ...
        <modal title="Exclusão de Situação de Pagamento" v-model="modalDeleteIsOpen" type="modal-danger" effect="fade/zoom">
            <div slot="modal-header" class="modal-header">
                <h4 class="modal-title">Exclusão de Situação de Pagamento</h4>
