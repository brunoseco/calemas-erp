<template>
    <modal title="Cadastro de Prioridade" v-model="crud.create.modalIsOpen" effect="fade/zoom" :success="true" :large="true">
        <div slot="modal-header" class="modal-header">
            <h4 class="modal-title">Cadastro de Prioridade</h4>
        </div>
        <form data-vv-scope="form-create" v-on:submit.prevent="crud.create.executeAction()">
            <fieldset>
                <legend>Dados</legend>
                <div class="row">
                    <input type="hidden" class="form-control" name="prioridadeId" v-model="crud.create.model.prioridadeId" />
					...
					<div class="form-group col-md-12" v-bind:class="{ 'has-danger': formErrors.has('form-create.cor') }">
                        <label for="cor">Cor</label>
                        <input type="text" class="form-control" name="cor" placeholder="Cor" v-model="crud.create.model.cor" v-validate="'required'" />
                    </div>
