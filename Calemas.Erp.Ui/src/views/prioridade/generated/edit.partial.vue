<template>
    <modal title="Edição de Prioridade" v-model="crud.edit.modalIsOpen" effect="fade/zoom" :primary="true" :large="true">
        <div slot="modal-header" class="modal-header">
            <h4 class="modal-title">Edição de Prioridade</h4>
        </div>
        <form data-vv-scope="form-edit" v-on:submit.prevent="crud.edit.executeAction()">
            <fieldset>
                <legend>Dados</legend>
                <div class="row">
                    <input type="hidden" class="form-control" name="prioridadeId" v-model="crud.edit.model.prioridadeId" />
                    ...
                    <div class="form-group col-md-12" v-bind:class="{ 'has-danger': formErrors.has('form-edit.corId') }">
                        <label for="corId">Cor</label>
                        <select v-select="{ dataitem: 'Cor', default: 'Selecione' }" v-model="crud.edit.model.corId" class="form-control" name="corId" v-validate="'required'"></select>
                    </div>
