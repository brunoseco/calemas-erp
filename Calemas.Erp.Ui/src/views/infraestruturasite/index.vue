<template>
    <div class="wrapper">
        ...
        <div class="row" style="margin-bottom: 1rem;margin-top: -0.5rem;">
            <div class="col-xs-12 col-sm-7 col-md-7 col-lg-8">
                <h6 class="page-title txt-color-blueDark">
                    ...
                    Cadastros
                    <span>Manutenção de Infraestrutura - Site</span>
                </h6>
            </div>
                    ...
                    <div class="card-block no-padding">
                        <table class="table has-tickbox table-striped table-sm">
                            <thead class="">
                                <tr>
                                    <th>#</th>
                                    <th>Nome<button @click="executeOrderBy('nome')" class="btn btn-xs btn-link no-border pull-right hide"><i class="fa fa-sort"></i></button></th>
                                    <th>Latitude<button @click="executeOrderBy('latitude')" class="btn btn-xs btn-link no-border pull-right hide"><i class="fa fa-sort"></i></button></th>
                                    <th>Longitude<button @click="executeOrderBy('longitude')" class="btn btn-xs btn-link no-border pull-right hide"><i class="fa fa-sort"></i></button></th>
                                    <th>Endpoint<button @click="executeOrderBy('endpoint')" class="btn btn-xs btn-link no-border pull-right hide"><i class="fa fa-sort"></i></button></th>
                                    <th>Login<button @click="executeOrderBy('login')" class="btn btn-xs btn-link no-border pull-right hide"><i class="fa fa-sort"></i></button></th>
                                    <th class="text-center" width="75"><i class="fa fa-cog"></i></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="item in result.itens" class="animated fadeIn">
                                    <td>{{ item.infraestruturaSiteId }}</td>
                                    <td>{{ item.nome }}</td>
                                    <td>{{ item.latitude }}</td>
                                    ...
                                    <td class="text-center">
                                        <button type="button" class="btn btn-sm btn-primary" v-tooltip.left="'Editar'" @click="openEdit(item.infraestruturaSiteId, item)">
                                            <i class="fa fa-pencil"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-danger" v-tooltip.left="'Remover'" @click="openDelete(item.infraestruturaSiteId, item)">
        ...
        <modal title="Cadastro de Infraestrutura - Site" v-model="modalCreateIsOpen" effect="fade/zoom" type="modal-success" :large="true">
            <div slot="modal-header" class="modal-header">
                <h4 class="modal-title">Cadastro de Infraestrutura - Site</h4>
        ...
        <modal title="Edição de Infraestrutura - Site" v-model="modalEditIsOpen" effect="fade/zoom" type="modal-primary" :large="true">
            <div slot="modal-header" class="modal-header">
                <h4 class="modal-title">Edição de Infraestrutura - Site</h4>
        ...
        <modal title="Exclusão de Infraestrutura - Site" v-model="modalDeleteIsOpen" type="modal-danger" effect="fade/zoom">
            <div slot="modal-header" class="modal-header">
                <h4 class="modal-title">Exclusão de Infraestrutura - Site</h4>
...
<script>
    ...
    export default {
        name: "infraestruturasite",
        components: { formPartial, filterPartial },
        mixins: [crudBase],
        data() {
            return {
                resource: "infraestruturasite",
            }
        }
    }
</script>
