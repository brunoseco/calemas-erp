        ...
        <div class="row" style="margin-bottom: 1rem;margin-top: -0.5rem;">
            <div class="col-xs-12 col-sm-7 col-md-7 col-lg-8">
                <h6 class="page-title txt-color-blueDark">
                    ...
                    Cadastros
                    <span>Manutenção de Cliente</span>
                </h6>
            </div>
                    ...
                    <div class="card-block no-padding">
                        <table class="table has-tickbox table-striped table-sm">
                            <thead class="">
                                <tr>
                                    <th>#</th>
                                    <th>Nome<button @click="executeOrderBy('pessoa.nome')" class="btn btn-xs btn-link no-border pull-right"><i class="fa fa-sort"></i></button></th>
                                    <th>Email<button @click="executeOrderBy('pessoa.email')" class="btn btn-xs btn-link no-border pull-right"><i class="fa fa-sort"></i></button></th>
                                    <th>Telefone<button @click="executeOrderBy('pessoa.telefone')" class="btn btn-xs btn-link no-border pull-right"><i class="fa fa-sort"></i></button></th>
                                    <th>Situação<button @click="executeOrderBy('account')" class="btn btn-xs btn-link no-border pull-right"><i class="fa fa-sort"></i></button></th>
                            ...
                            <tbody>
                                <tr v-for="item in result.itens" class="animated fadeIn">
                                    ...
                                    <td class="text-center">
                                        <button type="button" class="btn btn-xs btn-primary" @click="openEdit(item.clienteId, item)">
                                            <i class="fa fa-pencil"></i>
                                        </button>
                                        <button type="button" class="btn btn-xs btn-danger" @click="openDelete(item.clienteId, item)">
    ...
    export default {
        ...
        methods: {
            ...
            onBeforeCreate: function (model) {
                model.pessoa.apelido = model.pessoa.nome;
            },
        }
    }
</script>