<template>
    <modal title="Cadastro de Plano de Conta" v-model="crud.create.modalIsOpen" effect="fade/zoom" :success="true" :large="true">
        <div slot="modal-header" class="modal-header">
            <h4 class="modal-title">Cadastro de Plano de Conta</h4>
        </div>
        <form data-vv-scope="form-create" v-on:submit.prevent="crud.create.executeAction()">
            <fieldset>
                <legend>Dados</legend>
                <div class="row">
                    <input type="hidden" class="form-control" name="planoContaId" v-model="crud.create.model.planoContaId" />
                    ...
                    <div class="form-group col-md-12" v-bind:class="{ 'has-danger': formErrors.has('form-create.tipoPlanoContaId') }">
                        <label for="tipoPlanoContaId">TipoPlanoConta</label>
                        <select v-select="{ dataitem: 'TipoPlanoConta', default: 'Selecione' }" v-model="crud.create.model.tipoPlanoContaId" class="form-control" name="tipoPlanoContaId" v-validate="'required'"></select>
                    </div>
