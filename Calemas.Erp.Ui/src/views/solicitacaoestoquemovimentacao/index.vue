        ...
        <div class="row" style="margin-bottom: 1rem;margin-top: -0.5rem;">
            <div class="col-xs-12 col-sm-7 col-md-7 col-lg-8">
                <h6 class="page-title txt-color-blueDark">
                    ...
                    Solicitação
                    <span> de movimentação de estoque</span>
                </h6>
            </div>
                ...
                <div class="card">
                    ...
                    <div class="card-block no-padding">
                        <table class="table table-striped table-sm">
                            <thead class="">
                                <tr>
                                    <th>Estoque<button @click="executeOrderBy('estoque.nome')" class="btn btn-xs btn-link no-border pull-right hide"><i class="fa fa-sort"></i></button></th>
                                    <th>Quantidade<button @click="executeOrderBy('quantidade')" class="btn btn-xs btn-link no-border pull-right hide"><i class="fa fa-sort"></i></button></th>
                                    <th>Solicitante</th>
                                    ...
                                    <th>Entrada<button @click="executeOrderBy('entrada')" class="btn btn-xs btn-link no-border pull-right hide"><i class="fa fa-sort"></i></button></th>
                            ...
                            <tbody>
                                <tr v-for="item in result.itens" class="animated fadeIn">
                                    ...
                                    <td>{{ item.dataSolicitacao | date }}</td>
                                    ...
                                    <td class="text-center">
                                        <button type="button" class="btn btn-sm btn-primary" v-tooltip.left="'Editar'" @click="openEdit(item.solicitacaoEstoqueMovimentacaoId, item)">
                                            <i class="fa fa-pencil"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-danger" v-tooltip.left="'Remover'" @click="openDelete(item.solicitacaoEstoqueMovimentacaoId, item)">
...
<script>
    ...
    export default {
        name: "solicitacaoestoquemovimentacao",
        components: { formPartial, filterPartial },
        mixins: [crudBase],
        data() {
            return {
                resource: "solicitacaoestoquemovimentacao",
            }
        }
    }
</script>